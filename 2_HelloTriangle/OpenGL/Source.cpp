...
int main() {
	...
	*/
	...
	// ---- ELEMENT or INDEX BUFFER OBJECT (EBO or IBO) ---- //
	// This needs to be called before unbinding the VAO
	unsigned int EBO;
	glGenBuffers(1, &EBO);
	...
	while (!glfwWindowShouldClose(window)) {
		...
		// ---RENDERING COMMANDS---
		// Assin a color to GL_COLOR_BUFFER_BIT and color the 
		...
		// No need to call glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, IBO) because IBO was the last binded buffer at this target inside the VAO.
